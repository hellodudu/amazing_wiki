...
func (item *Document) ReleaseContent() error {
	...
	if err := os.RemoveAll(filepath.Join(conf.WorkingDirectory, "data/uploads", "books", strconv.Itoa(item.BookId))); err != nil {
		beego.Error("删除已缓存的文档目录失败 -> ", filepath.Join(conf.WorkingDirectory, "data/uploads", "books", strconv.Itoa(item.BookId)))
		return err
	}
